...
const drawStackChart = (() => {
  ...
  const canvasStackChart = d3.select('#Dash').append('svg')
  ...
  return (input) => {
    ...
    // console.log(colorStackChart.domain().slice().reverse());
  };
})();
// Line Chart
const drawLineChart = (() => {
...
// Using Promise
getData('data/crimesdata-theft.json').then(drawStackChart).catch((err) => {
  console.log(err);
});
...
(async function drawLine() {
  try {
    const dat = await getData('data/crimesdata-assault.json');
    drawLineChart(dat);
  } catch (err) {
    console.log(err);
  }
